...
void setup(){
  ...
  //Serial.println(F("DA16200 AT Command Example: Connecting to WiFi"));
  ...
  Serial.println(F("FIRMWARE IS IN FUNCTIONALITY MODE; DO NOT USE FOR DEPLOYMENT!!"));
  ...
  // to test the functionalities: gnss, waves, iridium
  if (true) {
    sleep_for_seconds(5);
    gnss_manager.get_and_push_fix(10UL);
    sleep_for_seconds(5);
    board_wave_analyzer.gather_and_analyze_wave_data();
    sleep_for_seconds(5);
    //iridium_manager.attempt_transmit_wave_spectra();
